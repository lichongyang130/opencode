...
export function LiveAuraOne() {
  return (
    <div className="h-full overflow-y-auto bg-[#080A0D] text-[#F4F6F8]">
      <article className="mx-auto max-w-[960px] px-5 py-10 sm:px-12 sm:py-14">
        <p className="text-[11px] font-medium tracking-[0.28em] text-[#65E8DD]/80">DESIGN / AURA ONE</p>
        <p className="mt-3 rounded-lg border border-[#65E8DD]/25 bg-[#65E8DD]/5 px-3 py-2 text-[12px] leading-6 text-[#B9C1CA]">
          本资料为概念产品介绍稿，产品外观、功能与规格均为规划设定。界面及显示效果为示意，不代表实际视场范围与画质。
        </p>

        <Page n="01" title="封面：视界之外，自有新境" dark>
          <Thesis>大屏体验，不必再困在桌面上。</Thesis>
          <Shot src="/cases/aura/p01-cover.jpg" cap="四分之三侧面 · 产品为唯一视觉中心" />
          <p className="mt-4 text-[13.5px] leading-7 text-[#B9C1CA]">
        ...
        <Page n="02" title="产品理念：大屏体验，不必困于桌面" dark>
        ...
        <Page n="03" title="工业设计：把复杂，藏进细节" dark>
        ...
        <Page n="04" title="显示体验：让内容，在眼前展开" dark>
        ...
        <Page n="05" title="空间体验：窗口有位置，工作有层次" dark>
        ...
        <Page n="06" title="个人影音：把沉浸感，留给自己" dark>
        ...
        <Page n="07" title="移动办公：连接之后，展开思路" dark>
          ...
          <ol className="mt-4 space-y-1.5 text-[13px] text-[#B9C1CA]">
            {["连接兼容主机", "打开已适配应用", "布置一主一辅窗口", "主机或键盘输入", "断开并收入盒中"].map((t, i) => (
              <li key={t} className="flex gap-2">
                <span className="font-mono text-[11px] text-[#65E8DD]">0{i + 1}</span>
                {t}
              </li>
            ))}
          </ol>
        ...
        <Page n="08" title="智能辅助：需要时，信息恰好出现" dark>
          ...
          <div className="mt-4 space-y-3">
            {[
              ...
              <div key={n} className="border border-white/10 p-3 text-[12px] text-[#B9C1CA]">
                <p className="font-semibold text-white">{n}</p>
        ...
        <Page n="09" title="交互设计：少一点操作，多一点直接" dark>
        ...
        <Page n="10" title="佩戴体验：舒适，来自每一处取舍" light>
        ...
        <Page n="11" title="系统架构：眼前简洁，背后有序" light>
        ...
        <Page n="12" title="参数与兼容性：每一项规格，都有边界" light>
        ...
        <Page n="13" title="产品配置：按场景，选择体验" light>
        ...
        <Page n="14" title="封底：下一块屏幕，不必放在桌上" dark>
          <Thesis>收进盒里，也是一种完成。</Thesis>
          <Shot src="/cases/aura/p14-case.jpg" cap="收拢状态与收纳盒，呼应封面" />
          <p className="mt-4 text-[14px] leading-8 text-[#B9C1CA]">
            我们不把下一块屏幕钉在墙上。曜界 AURA ONE 只做一件事：在你需要时，把画面轻轻放到眼前；在你结束时，安静回到盒中。
          </p>
          <div className="mt-6 flex flex-wrap gap-3 text-[13px]">
            <span className="rounded-full border border-[#65E8DD]/40 px-4 py-2 text-[#65E8DD]">了解概念方案</span>
            <span className="rounded-full border border-white/20 px-4 py-2">申请体验资讯</span>
          </div>
...
function Page({ n, title, dark, light, children }: { n: string; title: string; dark?: boolean; light?: boolean; children: ReactNode }) {
  return (
    <section className={`mt-12 border-t pt-8 ${light ? "border-stone-200 bg-[#F4F6F8] -mx-5 px-5 py-8 sm:-mx-12 sm:px-12" : "border-white/10"}`}>
      <p className={`font-mono text-[11px] tracking-[0.3em] ${light ? "text-[#65E8DD]" : "text-[#65E8DD]/80"}`}>{n}</p>
      <h2 className={`mt-2 text-[22px] font-semibold ${light ? "text-[#080A0D]" : "text-white"}`}>{title}</h2>
      {children}
    </section>
  );
}
...
function Thesis({ children }: { children: ReactNode }) {
  return <p className="mt-3 text-[14px] font-medium leading-7 text-[#65E8DD]">{children}</p>;
}

function Cards({ items, light }: { items: [string, string][]; light?: boolean }) {
  return (
    <div className="mt-5 grid gap-3 sm:grid-cols-3">
      {items.map(([h, b]) => (
        <div key={h} className={`p-3 ${light ? "border border-stone-200 bg-white" : "border border-white/10 bg-white/[0.03]"}`}>
          <p className={`text-[11px] tracking-wide ${light ? "text-[#0891a8]" : "text-[#65E8DD]/80"}`}>{h}</p>
          <p className={`mt-1.5 text-[12px] leading-6 ${light ? "text-[#3a4149]" : "text-[#B9C1CA]"}`}>{b}</p>
        </div>
      ))}
    </div>
  );
